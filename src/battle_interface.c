...
enum BattleCoordTypes GetBattlerCoordsIndex(u32 battlerId)
{
    if (GetBattlerPosition(battlerId) == B_POSITION_PLAYER_LEFT && gPlayerPartyCount == 1 && !(gBattleTypeFlags & BATTLE_TYPE_MULTI))
        return BATTLE_COORDS_SINGLES;
    else if (GetBattlerPosition(battlerId) == B_POSITION_OPPONENT_LEFT && gEnemyPartyCount == 1 && !(gBattleTypeFlags & BATTLE_TYPE_TWO_OPPONENTS))
        return BATTLE_COORDS_SINGLES;
...
void GetBattlerHealthboxCoords(u8 battlerId, s16 *x, s16 *y)
{
    u8 position = GetBattlerPosition(battlerId);
    enum BattleCoordTypes index = GetBattlerCoordsIndex(battlerId);
...
void InitBattlerHealthboxCoords(u8 battlerId)
{
    ...
    UpdateSpritePos(gHealthboxSpriteIds[battlerId], x, y);
}
...
static void UpdateLvlInHealthbox(u8 healthboxSpriteId, u8 lvl)
{
    ...
    if (GetIndicatorPalTag(battlerId) != TAG_NONE)
    {
...
void UpdateHpTextInHealthbox(u32 healthboxSpriteId, u32 maxOrCurrent, s16 currHp, s16 maxHp)
{
    ...
    case BATTLE_COORDS_SINGLES:
    {
        if (IsOnPlayerSide(battlerId)) // Player
        {
...
{
    for (u32 battlerId = 0; battlerId < gBattlersCount; battlerId++)
    {
